...
// initialize map view
const map = L.map('map').setView([13.042009, 80.249360], 12);
...
function onEachFeature(feature, layer) {
    ...
    layer.bindPopup('<h4>Litter Risk</h4>' +  
                    (feature.properties ?
                     '<b>' + feature.properties.count + ' count' 
                     + '</b><br />' + feature.properties.watr_nn.toFixed(2) + 'm /water_nn'
                     + '</b><br />' + feature.properties.restaurant_nn.toFixed(2) + 'm /restaurant_nn'
                     : 'No data available'));
...
chennai.features.forEach(feature => {
...
// Iterate over the overlayMaps object and set checked property to true for each layer
Object.keys(overlayMaps).forEach(function (key) {
    overlayMaps[key].addTo(map); // Add each layer to the map
    map.addLayer(overlayMaps[key]); // Ensure layer is added to the map
});
